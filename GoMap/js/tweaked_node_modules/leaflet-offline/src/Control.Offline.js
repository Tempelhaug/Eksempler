...
(function(factory, window) {
  ...
  L.Control.Offline = L.Control.extend({
     ...
     */
    initialize(baseLayer, tilesDb, options) {
     ...
     */
    _saveTiles(id) {
      const saveTilesPromise = new Promise((resolve, reject) => {
        var self = this;
        let mapId = id;
        var bounds = null;
        var zoomLevels = [];
        ...
        let maxZoomlevels = currentZoom + this.options.maxOfflineZoomLevels - 1;
        if (currentZoom + this.options.maxOfflineZoomLevels - 1 > maxZoom) {
          maxZoomlevels = maxZoom;
        }
        for (var zoom = currentZoom; zoom <= maxZoomlevels; zoom++) {
          zoomLevels.push(zoom);
        }

        for (var i = 0; i < zoomLevels.length; i++) {
          bounds = L.bounds(this._map.project(latlngBounds.getNorthWest(), zoomLevels[i]), this._map.project(latlngBounds.getSouthEast(), zoomLevels[i]));
          tileUrls = tileUrls.concat(this._baseLayer.getTileUrls(bounds, zoomLevels[i]));
        }
        const offlineBounds = this._map.getBounds();
        var continueSaveTiles = function() {
          self._baseLayer.fire('offline:save-start', {
            tilesLength: tileUrls.length,
          });
          self._tilesDb
            .saveTiles(mapId, tileUrls)
            .then(function() {
              resolve({ tileUrlLength: tileUrls.length, offlineBounds, currentZoom, maxZoomlevels });
