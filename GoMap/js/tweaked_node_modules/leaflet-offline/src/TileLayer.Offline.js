...
(function(factory, window) {
  ...
  L.TileLayer.Offline = L.TileLayer.extend({
     ...
     */
    initialize(url, tilesDb, mapId, options) {
      this._url = url;
    ...
    getTileUrl(coords) {
      ...
      const dbStorageKey = this._getStorageKey(url);
      const resultPromise = this._tilesDb
        .getItem(dbStorageKey, this.mapId)
        .then(function(data) {
          if (data && self.mapId) {
    ...
    getTileUrls(bounds, zoom) {
      ...
      const tileBounds = L.bounds(bounds.min.divideBy(this.getTileSize().x).floor(), bounds.max.divideBy(this.getTileSize().x).floor());
    ...
    _getStorageKey(url) {
      ...
      if (url.indexOf('{s}')) {
        const regexstring = new RegExp(`[${this.options.subdomains.join('|')}].`);
   ...
   * for up-to-date information.
   */
  L.tileLayer.offline = function(url, tilesDb, mapId, options) {
    return new L.TileLayer.Offline(url, tilesDb, mapId, options);
  };
}, window);
